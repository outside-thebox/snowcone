    ...
    <div v-show="lista.length > 0">
        ...
        <table class="table responsive table-bordered table-hover table-striped" style="margin-top: 10px" >
            ...
            <tr v-for="registro in lista" class="@{{ registro.deleted_at ? 'inactivo' : '' }}">
                ...
                <td>
                    <a title='Editar' href="{{route('sucursales.index')}}/@{{ registro.id }}/edit"><i class='glyphicon glyphicon-edit' ></i></a>
                    <a v-show="!registro.deleted_at" title='Desactivar' style="cursor: pointer" @click='desactivar(registro.id,registro.nombre)' ><i class='glyphicon glyphicon-trash' ></i></a>
                    <a v-show="registro.deleted_at" title='Activar' style="cursor: pointer" @click='activar(registro.id,registro.nombre)' ><i class='glyphicon glyphicon-thumbs-up' ></i></a>
                </td>
            </tr>
            </tbody>
        </table>
